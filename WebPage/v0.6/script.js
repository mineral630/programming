...
// --- [주의] API 키 (보안상 민감하므로 실제 배포 시에는 서버 측에서 처리해야 합니다) ---
...
const deviceSymptoms = {
  ...
  '전문가 수준': [
    ...
    '최신 전문가 자료를 활용하여 지원합니다.'
  ]
};

// --- 채팅 시작 시 사용할 현재 기기 이름 저장 변수 ---
let currentDeviceForChat = '';

/**
 * 챗봇 페이지로 전환하고 초기 메시지를 설정하는 함수
 * @param {string} deviceName - 선택된 기기/기능 이름
 */
function startChatting(deviceName) {
  ...
  const initialMsg = `안녕하세요! AI 진단 봇입니다. 선택하신 '${deviceName}'에 대해 무엇이 궁금하신가요? 구체적인 증상을 말씀해주세요.`;
  addMessage(initialMsg, 'assistant');
  chatInput.focus();
}

// '챗봇 시작하기' (일반) 버튼 리스너 - 즉시 진단으로 모달 띄우기
...
async function sendMessage() {
  ...
  // 3. 프롬프트 구성 (선택된 기기 정보를 포함하여 더 나은 답변 유도)
  const prompt = `사용자가 '${currentDeviceForChat}'에 대해 문의했습니다. 사용자의 질문: "${msg}". 이 기기/기능에 맞춰 친절하고 전문적인 진단 및 해결책을 제시해 주세요.`;
...
modalContinueBtn.addEventListener('click', () => {
  // 모달을 통해 선택한 기기 이름으로 챗봇 시작
  startChatting(currentDeviceForChat); 
});
...
modalOverlay.addEventListener('click', (e) => {
  // 클릭된 요소가 오버레이 자체인지 확인 (모달 내용 클릭 방지)
