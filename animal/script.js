...
// MBTI 기능
let mbtiScore = { E: 0, I: 0 };

function openMbtiModal() { 
    // 점수 및 화면 초기화
    mbtiScore = { E: 0, I: 0 };
    document.querySelectorAll('.mbti-step').forEach(el => el.style.display = 'none');
    document.getElementById('mbti-result').style.display = 'none';
    
    document.getElementById('mbti-step-1').style.display = 'block';
    document.getElementById('mbti-modal').style.display = 'block'; 
}
...
function nextMbti(currentStep, type) {
    // 선택한 타입에 점수 추가
    mbtiScore[type]++;

    // 현재 질문 숨기기
    document.getElementById(`mbti-step-${currentStep}`).style.display = 'none';

    // 다음 단계로 이동
    if (currentStep < 3) {
        document.getElementById(`mbti-step-${currentStep + 1}`).style.display = 'block';
    } else {
        showMbtiResult();
    }
}

// script.js의 showMbtiResult 함수 수정

function showMbtiResult() {
    const resultDiv = document.getElementById('mbti-result');
    const iconDiv = document.getElementById('mbti-result-icon');
    const textDiv = document.getElementById('mbti-result-text');
    const descDiv = document.getElementById('mbti-result-desc');

    // 결과 판별 (E가 많으면 활발, I가 많으면 차분)
    if (mbtiScore.E > mbtiScore.I) {
        iconDiv.innerText = "🐶";
        textDiv.innerText = "활발한 에너자이저 댕댕이!";
        descDiv.innerText = "당신은 활동적인 친구와 잘 어울려요.\n산책과 놀이를 좋아하는 아이를 찾아보세요!";
    } else {
        iconDiv.innerText = "🐱";
        textDiv.innerText = "조용하고 우아한 냥이!";
        descDiv.innerText = "당신은 차분한 친구와 잘 어울려요.\n곁에서 조용히 위로가 되어줄 아이를 찾아보세요!";
    }
    
    const resultBtn = resultDiv.querySelector('button');
    if (resultBtn) {
        resultBtn.className = 'more-btn'; // 클래스 변경
    }
